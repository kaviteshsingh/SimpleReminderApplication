...
HANDLE hProcess;
HWND hStatusBar;
HMENU ghMenu,hSystemTrayMenu,hSystemMenu;
...
LRESULT CALLBACK WndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
			...
			hListViewControl = CreateListViewBox(ghInstance,hwnd);
	 ...
	 case WM_COMMAND:
		 {
			 ...
			 case  IDM_SYS_ADD_REMINDER:
				 ...
				 DialogBoxW(ghInstance,MAKEINTRESOURCE(IDD_DIALOG1),hwnd,DialogProc);
				 return 0;
			 ...
			 case IDM_APP_STARTUP:
				 {
					 ...
					 if(ApplicationStartUpStatus)
					 {
						 ...
						 hOptionMenu=GetSubMenu(ghMenu,1);
...
void CreateTrayIcon(const HWND hwnd)
{
	HICON hIcon=LoadIcon(ghInstance, MAKEINTRESOURCE(IDI_ICON2));
